        ...
        for (int i = 0; i < ghostData.length && i < ghostRectangles.length; i++) {
            for (int j = 0; j < ghostData[i].length && j < ghostRectangles[i].length; j++) {
                ...
                if (boardRow >= hiddenRowCount && cell != 0) {
                    ...
                    ghostRectangles[i][j].setStrokeWidth(GameConstants.GHOST_STROKE_WIDTH);
                    ghostRectangles[i][j].setArcHeight(GameConstants.RECT_ARC_RADIUS);
                    ghostRectangles[i][j].setArcWidth(GameConstants.RECT_ARC_RADIUS);
                }
            }
        }
    }
